...
- On Windows systems, be familiar with phishing-delivered malware, malicious PowerShell activity, suspicious services or scheduled tasks, credential dumping attempts, privilege escalation, lateral movement with remote administration tools, and persistence through registry or startup changes.
- On Linux systems, understand attacks such as SSH brute forcing, abuse of sudo or weak permissions, malicious cron jobs, web shell deployment, unauthorized user creation, persistence through startup scripts, and suspicious command execution tied to crypto mining or remote access.
...
![TryHackMe SIEM dashboard](images/image.png)